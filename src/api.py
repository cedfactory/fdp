...
def api_history(str_exchange, str_symbol, str_start, str_end = None, str_interval = "1d", length = None):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {executor.submit(crypto.get_symbol_ohlcv, exchange_name=str_exchange, symbol=real_symbol, start=str_start, end=str_end, timeframe=str_interval, length=length): real_symbol for real_symbol in real_symbols}
